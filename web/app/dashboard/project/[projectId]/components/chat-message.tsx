...
export function ChatMessages({
  ...
  // messages come from chat-panel.tsx's own useQuery (the real, subscribed
  // one). useChat only supplies in-flight streaming state + the send/stop
  // actions — it does not own or read the persisted message list itself.
  const { streamingMessage, thinkingMessage, isLoading, sendMessage, stop } =
    useChat({
      projectId,
      chatId,
    });
        ...
        <div className="mx-auto max-w-2xl">
          {messages.length === 0 && !streamingMessage ? (
            ...
            <div className="space-y-4 pb-4">
              ...
              {isLoading && thinkingMessage && !streamingMessage && (
                <div className="flex justify-start">
                  <div className="max-w-[70%] rounded-md px-3.5 py-2.5">
                    <div className="flex items-center gap-2">
              ...
              {isLoading && !streamingMessage && !thinkingMessage && (
                <LoadingSkeleton />
              )}
            </div>
          )}
