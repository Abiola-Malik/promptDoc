 ...
 */
export async function cleanupTempDir(tempDir: string): Promise<{ success: boolean; error?: string }> {
 ...
 throw new Error('tempDir parameter is required');
  ...
  if (!tempDir.includes('temp') && !tempDir.includes('tmp')) {
    throw new Error(`Warning: Attempting to delete directory that doesn't appear to be temporary: ${tempDir}`);
  }
