...
import { ChunkOptions, CodeChunk, ExtractedFile, ExtractionResult } from "@/types/global"
import AdmZip from "adm-zip"
import fs from "fs/promises"
import path from "path"
import { detectChunkType, detectCodeBlocks, getLanguageFromFilename } from "../utils"
...
export const chunkCode = (
  ...
  // Detect code block boundaries (functions, classes, etc.)
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    if (
      ...
      currentChunk = overlapLines > 0 
        ? currentChunk.slice(-overlapLines) 
        : []
